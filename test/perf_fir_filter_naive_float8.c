#include <sys/time.h>
#include <stdio.h>
#include "../src/fir_filter_naive_float8.h"
...
int main(void) {
    ...
    fir_filter_naive_float8 *filter = NULL;
    int code = fir_filter_naive_float8_create(2016000 / 48000, taps, taps_len, input_len, &filter);
    if (code != 0) {
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < total_executions; i++) {
        fir_filter_naive_float8_process(input, input_len, &output, &output_len, filter);
    }
    ...
    printf("\n");
    fir_filter_naive_float8_destroy(filter);
