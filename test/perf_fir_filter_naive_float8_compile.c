#include <sys/time.h>
#include <stdio.h>
#include "../src/fir_filter_naive_float8_compile.h"
...
int main(void) {
    ...
    fir_filter_naive_float8_compile *filter = NULL;
    int code = fir_filter_naive_float8_compile_create(2016000 / 48000, taps, taps_len, input_len, &filter);
    if (code != 0) {
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < total_executions; i++) {
        fir_filter_naive_float8_compile_process(input, input_len, &output, &output_len, filter);
    }
    gettimeofday(&end, NULL);
    double time_spent = (double) (end.tv_usec - start.tv_usec) / 1000000 + (double) (end.tv_sec - start.tv_sec);

    printf("average time: %f of total %f\n", time_spent / total_executions, time_spent);
    for (int i = 0; i < 20; i++) {
        printf("%.9f, %.9f ", crealf(output[i]), cimagf(output[i]));
    }
    printf("\n");
    fir_filter_naive_float8_compile_destroy(filter);

    // Raspberrypi 3
    // average time: 0.014265
