#include "fir_filter_naive_float8_local.h"
#include <errno.h>
#include <string.h>
#include <complex.h>
#include <stdio.h>
...
struct fir_filter_naive_float8_local_t {
    uint8_t decimation;
...
int fir_filter_naive_float8_local_create(uint8_t decimation, float complex *taps, size_t taps_len, size_t max_input_buffer_length, fir_filter_naive_float8_local **filter) {
    struct fir_filter_naive_float8_local_t *result = malloc(sizeof(struct fir_filter_naive_float8_local_t));
    if (result == NULL) {
        return -ENOMEM;
    }
    // init all fields with 0 so that destroy_* method would work
    *result = (struct fir_filter_naive_float8_local_t) {0};
    ...
    if (result->output == NULL) {
        fir_filter_naive_float8_local_destroy(result);
        return -ENOMEM;
    }
    ...
    fp = fopen("../fir_filter_naive_float8_local.cl", "r");
    if (!fp) {
...
// Create memory buffers on the device for each vector
    ...
    ret = clSetKernelArg(result->kernel, 5, 12 * sizeof(float complex), NULL);
    printf("clSetKernelArg: %d\n", ret);
    if (ret != 0) {
        fir_filter_naive_float8_local_destroy(result);
        return ret;
