#include "fir_filter_naive_float8_memmap.h"
#include <errno.h>
#include <string.h>
#include <complex.h>
#include <stdio.h>
...
struct fir_filter_naive_float8_memmap_t {
    uint8_t decimation;
...
int fir_filter_naive_float8_memmap_create(uint8_t decimation, float complex *taps, size_t taps_len, size_t max_input_buffer_length, fir_filter_naive_float8_memmap **filter) {
    struct fir_filter_naive_float8_memmap_t *result = malloc(sizeof(struct fir_filter_naive_float8_memmap_t));
    if (result == NULL) {
        return -ENOMEM;
    }
    // init all fields with 0 so that destroy_* method would work
    *result = (struct fir_filter_naive_float8_memmap_t) {0};
    ...
    if (result->output == NULL) {
        fir_filter_naive_float8_memmap_destroy(result);
        return -ENOMEM;
    }
