#include "fir_filter_naive_float8_compile.h"
#include <errno.h>
#include <string.h>
#include <complex.h>
#include <stdio.h>
...
struct fir_filter_naive_float8_compile_t {
    uint8_t decimation;
...
int fir_filter_naive_float8_compile_create(uint8_t decimation, float complex *taps, size_t taps_len, size_t max_input_buffer_length, fir_filter_naive_float8_compile **filter) {
    struct fir_filter_naive_float8_compile_t *result = malloc(sizeof(struct fir_filter_naive_float8_compile_t));
    if (result == NULL) {
        return -ENOMEM;
    }
    // init all fields with 0 so that destroy_* method would work
    *result = (struct fir_filter_naive_float8_compile_t) {0};
    ...
    if (result->output == NULL) {
        fir_filter_naive_float8_compile_destroy(result);
        return -ENOMEM;
    }
    ...
    fp = fopen("../fir_filter_naive_float8_compile.cl", "r");
    if (!fp) {
...
// Create memory buffers on the device for each vector
    ...
    ret = clBuildProgram(result->program, 1, &result->device_id, source_str, NULL, NULL);
    ...
    ret = clSetKernelArg(result->kernel, 2, sizeof(cl_mem), (void *) &result->output_obj);
    printf("clSetKernelArg: %d\n", ret);
    if (ret != 0) {
        fir_filter_naive_float8_compile_destroy(result);
        return ret;
...
int fir_filter_naive_float8_compile_process(const float complex *input, size_t input_len, float complex **output, size_t *output_len, fir_filter_naive_float8_compile *filter) {
    ...
    cl_int ret = clEnqueueWriteBuffer(filter->command_queue, filter->input_obj, CL_TRUE, 0,
